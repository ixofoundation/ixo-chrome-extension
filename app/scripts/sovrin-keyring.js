...
class SovrinKeyring extends EventEmitter {
  ...
  _getWalletForAccount (account) {
    ...
    const targetAddress = account.substring(IXO_DID_PREFIX.length);
    return this.wallets.find((w) => {
      const address = w.did;
      return ((address === targetAddress) ||
              (address.substring(IXO_DID_PREFIX.length) === targetAddress));
    });
  }
}
